...
def test_to_tardis_date():
    assert to_tardis_date(pd.Timestamp("2022-01-01 05:12")) == "2022-01-01"


def test_convert_to_ohlc(tardis_quotes_1: pd.DataFrame):
...
def test_resample_and_merge(
    ...
    assert (
        result.open[0] == result.high[0] == result.low[0] == result.close[0] == 1691.815
    )
...
def test_extension(mocker, zipline_environment):
