...
@lru_cache
def tardis_regex(exchange: str = "[a-zA-Z0-9]+") -> Pattern:
    return re.compile(
...
def _date_as_str(tardis_filename: str) -> str:
    return tardis_regex().match(tardis_filename).group(1)


def date_of(tardis_filename: str) -> datetime.date:
...
def _clean_quotes_data(data: _ResampleData) -> _ResampleData:
    ...
    return data.new_data(
        data.dfr[data.dfr.index.map(lambda x: x.date()) == date_of(data.filename)]
    )
